		...
		<div className="flex flex-col h-[10vh] bg-gray-100">
			<header className="flex items-center justify-between px-8 py-4 bg-white shadow-md lg:shadow-none">
				<Link to='/home' className='flex items-center justify-center gap-2'>
					<img src={require('../assert/logo.ico')} alt="" className="h-14 object-fill" />
					<h1 className=" md:flex hidden text-2xl text-primary font-bold">Cloud Point</h1>
				</Link>
				<div className="bg-gray-200 hidden lg:flex p-1 rounded-full gap-3 items-center justify-center">
					{links.map(item => (
						<Link to={item.link} className={`px-5 py-2 rounded-full ${location.pathname.includes(item.link) ? 'bg-white active:scale-90 transition-all  font-semibold text-primary' : ''
							} `}>
							{item.name}
						</Link>
					))}
				...
				<div className='flex items-center ml-2 justify-center'>
					{user ? <Link to='/dashboard' className="px-5 py-2  active:scale-90 border-2 border-white outline outline-[3px] outline-blue-500 transition-all bg-blue-500 text-center text-white rounded-full">
					Your Space
					</Link> : <Link to='/login' className="px-5 py-2  active:scale-90 border-2 border-white outline outline-[3px] outline-blue-500 transition-all bg-blue-500 text-center text-white rounded-full">
					Access Now
					</Link>}
